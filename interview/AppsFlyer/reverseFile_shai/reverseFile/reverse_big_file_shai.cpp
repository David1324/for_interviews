...
void reverse_file(const char* fileFullPath)
{
	...
	std::streampos endOffset = fileSize;
	int blockSize;
	while (offset < fileSize / 2)
	{
		...
		myFile.seekp(-(offset + blockSize), myFile.end);

		int endWroteTo = myFile.tellp();
		...
		int temp = myFile.tellp();
		if (blockSize == 1 && (leftBlock[blockSize - 1] == '\n' && rightBlock[0] == '\n') && endWroteTo - temp == 1)
		{
			offset += blockSize;
			continue;
		}
