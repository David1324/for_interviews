...
void histogram_words_instances(const char* fileFullPath, std::unordered_map<std::string, int> &wordsHistogram)
{
...
std::unordered_multimap<int, std::string> flip_map(const std::unordered_map<std::string, int> &src)
{
	std::unordered_multimap<int, std::string> dst;
	//std::transform(src.begin(), src.end(), std::inserter(dst, dst.begin()), flip_pair);

	std::unordered_map<std::string, int>::const_iterator firstIterator = src.begin();
	std::unordered_map<std::string, int>::const_iterator lastIterator = src.end();
	std::insert_iterator<std::unordered_multimap<int, std::string>> destIterator = std::inserter(dst, dst.begin());
...
int main()
{
	std::string fileName = "song_un_ordered_map.txt";
	reverse_file(fileName.c_str());

	std::unordered_map<std::string, int> wordsHistogram;
	histogram_words_instances(fileName.c_str(), wordsHistogram);

	int countFrequentWords = 0;
	std::unordered_map<std::string, int>::iterator it;
	for (it = wordsHistogram.begin(); it != wordsHistogram.end(); ++it)
	{
		countFrequentWords++;
		std::cout << countFrequentWords + 1 << ".\t" << it->second << ": " << it->first << '\n';
	}
	...
	//std::unordered_multimap<int, std::string> wordsSortedByOccurences = flip_map(wordsHistogram);

	////using reverse_iterator to print from biggest number to smallest number
	//std::unordered_multimap<int, std::string>::reverse_iterator rit;
	//int countFrequentWords = 0;
