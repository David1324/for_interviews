function EvalRR()
    ...
    % Raw data
    CalculateRRCharacteristics('raw',GridX,GridY,ValidX,ValidY,CorrCoef,size(ValidX,1));

    if IncludeCorr
        CommonIndicesIndicator=ismember(ValidX,ValidXCorr,'rows');
       
        % Corrected data
        CalculateRRCharacteristics('corr',GridX,GridY,ValidX,ValidY,CorrCoef,CommonIndicesIndicator);
    end
end

function CalculateRRCharacteristics(Type,GridX,GridY,ValidX,ValidY,CorrCoef,CommonIndicesIndicator)
    ...
    xlim([0,1]);
    DataFileName=['corrcoef_distdata',Type,'.dat'];
    Delimiter='\t';
    ...
    PlotSubFigure(Sub1,['xdispl',Type,'.png'],1);
    PlotSubFigure(Sub2,['ydispl',Type,'.png'],1);
    PlotSubFigure(Sub3,['corrcoef',Type,'.png'],1);
    PlotSubFigure(Sub4,['corrcoef_dist',Type,'.png'],0);
    PlotSubFigure(Sub5,['displx_dist',Type,'.png'],0);
    PlotSubFigure(Sub6,['disply_dist',Type,'.png'],0);  
    saveas(Figure,['RR',Type,'.png'],'png');
    saveas(Figure,['RR',Type,'.fig'],'fig');
    close(Figure);
end
