...
function [ValidX,ValidY,DisplX,DisplY]=RTCorrelation(GridXCoarse,GridYCoarse,GridXFine,GridYFine)
    ...
    % Get image sequence
    [FirstImageName,ImageFolder]=uigetfile({'*.bmp;*.tif;*.jpg;*.jpeg;*.png','Image files (*.bmp,*.tif,*.jpg,*.jpeg;*.png)';'*.*','All Files (*.*)'},'Open First Image');
    ...
    if ~isempty(FirstImageName)
        ...
        while exist(EndFile,'file')==0
            ...
            if exist(CurrentFileName,'file')==2
                ...
                [InputCorrXReduced,InputCorrYReduced,CurrentCorrCoefReduced] = CallCpcorr(InputPointsXReduced,InputPointsYReduced,BasePointsXReduced,BasePointsYReduced,DisplXTemp,DisplYTemp,InputResized,BaseResized,CpCorrData);
                ...
                % Fine analysis
                [InputCorrX,InputCorrY,CurrentCorrCoef] = CallCpcorr(InputPointsX,InputPointsY,BasePointsX,BasePointsY,DisplXTemp,DisplYTemp,Input,Base,CpCorrData);
...
function [InputCorrX,InputCorrY,CorrCoef] = Cpcorr(InputPointsX,InputPointsY,BasePointsX,BasePointsY,DisplX,DisplY,Input,Base,CpCorrData)
...
function [InputCorrX,InputCorrY,CorrCoef] = CpcorrLocal(InputPointsX,InputPointsY,BasePointsX,BasePointsY,DisplX,DisplY,Input,Base,CpCorrData)    
   
    InputPoints=[InputPointsX+DisplX,InputPointsY+DisplY];
    BasePoints=[BasePointsX,BasePointsY];
    [InputCorr,CorrCoef]=feval(CpCorrData.FunctionFileName,CpCorrData.CorrSize,round(InputPoints),round(BasePoints),Input,Base); 
    InputCorrX=InputCorr(:,1); % results from cpcorr for the x-direction
    InputCorrY=InputCorr(:,2); % results from cpcorr for the y-direction      
    
% Get average strain by linear regression (comapre original data with prediction)
