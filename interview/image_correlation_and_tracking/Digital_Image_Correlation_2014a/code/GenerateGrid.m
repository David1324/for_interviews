...
function [GridX,GridY,FileNameBase,PathNameBase]=GenerateGrid(FileNameBase,PathNameBase,GridX,GridY)
    ...
    if exist('FileNameBase','var')~=1 || isempty(FileNameBase)
        [FileNameBase,PathNameBase,FilterIndex] = uigetfile({'*.bmp;*.tif;*.jpg;*.jpeg;*.png','Image files (*.bmp,*.tif,*.jpg,*.jpeg;*.png)';'*.*','All Files (*.*)'},'Open base image for grid creation');
    else 
        FilterIndex=1;
    end
...
function [GridX,GridY]=SelectGridType(FileNameBase,PathNameBase,BaseImage,GridX,GridY,LoadGrid)
    ...
    % Select grid
    GridSelection = menu(sprintf('Gridgenerator Main Menu'),'Rectangular','Circular','Two Markers','Line','Remove Markers','END');
