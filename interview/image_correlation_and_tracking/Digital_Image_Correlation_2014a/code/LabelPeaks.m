...
% Written by Chris
% Revised by Melanie
% Last revision: 04/16/14

function [ValidX,ValidY]=LabelPeaks

    [ImageName,PathImage]=uigetfile({'*.bmp;*.tif;*.jpg;*.jpeg;*.png','Image files (*.bmp,*.tif,*.jpg,*.jpeg;*.png)';'*.*','All Files (*.*)'},'Open image for peak labelling');
    cd(PathImage);
    ...
    for CurrentLabel=1:NumOfCroppedLabels 
        ...
        [X,ResNormX,ExitFlagX]=FitPeak(CroppedImage,XDirectionPoints,CroppedLabelsXY,CurrentLabel,XDirectionSum,ShowFit);
        
        % Fitting in y-direction
        [Y,ResNormY,ExitFlagY]=FitPeak(CroppedImage,YDirectionPoints,CroppedLabelsXY,CurrentLabel,YDirectionSum,ShowFit);
    ...
    for CurrentImage=1:NumOfImages-1
        tic;
        LabelCount=0;
        Image=mean(double(imread(FileNameList(CurrentImage,:))),3);
        ...
        for CurrentLabel=1:NumOfFittedLabels % for all fitted labels
            ...
            [X,ResNormX,ExitFlagX]=ReFitPeak([AmplitudeGuessX PositionGuessX WidthGuessX BackgroundGuessX],CropX,CropY,CroppedImage,FileNameList(CurrentImage,:),1,ShowFit, ...
                                             TotalTimeHours,EstimatedTotalTimeHours,TotalProgress);
            
            % Refitting in y-direction
            [Y,ResNormY,ExitFlagY]=ReFitPeak([AmplitudeGuessY PositionGuessY WidthGuessY BackgroundGuessY],CropY,CropY,CroppedImage,FileNameList(CurrentImage,:),2,ShowFit, ...
                                             TotalTimeHours,EstimatedTotalTimeHours,TotalProgress);
