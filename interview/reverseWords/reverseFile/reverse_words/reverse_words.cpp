...
void reverseSentence(std::string& sentence)
{
	const std::string delims = " \t,.;\r";
	std::string::size_type indexStartOfWord, indexEndOfWord;
	indexStartOfWord = sentence.find_first_not_of(delims);

	std::string currentWord;
	while (indexStartOfWord != std::string::npos) {
		indexEndOfWord = sentence.find_first_of(delims, indexStartOfWord);
		if (indexEndOfWord == std::string::npos) { //if no matches are found
			indexEndOfWord = sentence.length(); //in this case this is the last word in line
		}
		currentWord.assign(sentence, indexStartOfWord, indexEndOfWord - indexStartOfWord);

		reverseWord(currentWord);

		sentence.replace(indexStartOfWord, indexEndOfWord - indexStartOfWord, currentWord);

		indexStartOfWord = sentence.find_first_not_of(delims, indexEndOfWord);
...
int main()
{
	std::string sentence = "hello my name is David. how are you?";
	std::string str2 = "12345678901234567890";
	//int len = sentence.length();
	//str2.replace(0, len, sentence);
