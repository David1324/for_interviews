...
Thread::Thread(std::auto_ptr<Runnable> r) : runnable(r) {
	if(!runnable.get())
		printError((LPTSTR)"Thread(std::auto_ptr<Runnable> r) failed at ",
				__FILE__, __LINE__);
	hThread = 
		(HANDLE)_beginthreadex(NULL,0,Thread::startThreadRunnable,
				(LPVOID)this, CREATE_SUSPENDED, &wThreadID);
	if(!hThread)
		printError((LPTSTR)"_beginthreadex failed at ",__FILE__, __LINE__);
}
...
Thread::~Thread() {
	if(wThreadID != GetCurrentThreadId()) {
		...
		if(!rc) printError
			((LPTSTR)"CloseHandle failed at ",__FILE__, __LINE__);
	}
}
...
void Thread::start() {
	...
	if(!rc) printError
			((LPTSTR)"ResumeThread failed at ",__FILE__, __LINE__);
}
...
void Thread::printError(LPTSTR lpszFunction, LPSTR fileName, int lineNumber)
{
	...
	FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER||
		...
		0,
		NULL);
	wsprintf(szBuf,(LPCWSTR)"%s failed at line %d in %s with error %d: %s", 
...
public:
	...
	virtual void* run() {
		std::cout << "(simpleRunnable): Thread " << myID << " is running" << std::endl;
...
public:
	...
	virtual void* run() {
		std::cout << "(simpleThread): Thread " << myID << " is running" << std::endl;
		return reinterpret_cast<void*>(myID);
	}
private:
	int myID;
